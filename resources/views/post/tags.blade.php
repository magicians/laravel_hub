        ...
        <div class="row">
            <div class="col-md-8">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <p class="panel-title font-14"><span><i class="fa fa-list"></i></span>&nbsp;&nbsp;当前节点 - {{ $name }}
                            <a href="{{ action('PostController@essential') }}"><span class="pull-right"><i class="fa fa-bookmark"></i> 查看精华</span></a>
                        </p>
                    </div>
            ...
            <div class="col-md-4">
                        ...
                        @if(isset($hot))
                            @for($i=0;$i<count($hot);$i++)
                                <li class="list-group-item">
                                    ...
                                    <div class="media">
                                        <div class="media-left">
                                            <a href="{{ action('ProfileController@index',['id'=>$discuz->find($hot[$i]['hotPostId'])->user->id]) }}">
                                                <img class="media-object img-thumbnail" src="{{ url($discuz->find($hot[$i]['hotPostId'])->user->avatar) }}" width="48">
                                            </a>
                                        </div>
                                        <div class="media-body">
                                            @if(strlen($hot[$i]['hotPostTitle'])>50)
                                                <a class="media-heading" href="{{ url('/post/'.$hot[$i]['hotPostId']) }}">{{ mb_substr($hot[$i]['hotPostTitle'],0,50).'...' }}</a>
                                            @else
                                                <a class="media-heading" href="{{ url('/post/'.$hot[$i]['hotPostId']) }}">{{ $hot[$i]['hotPostTitle'] }}</a>
                                            @endif
                                            <br/>
                                            <div class="media-meta margintop-10"><a href="{{ action('ProfileController@index',['id'=>$discuz->find($hot[$i]['hotPostId'])->user->id]) }}">{{ $discuz->findOrfail($hot[$i]['hotPostId'])->user->name }}</a>・<span
                                                        class="label label-danger">热度: {{ $hot[$i]['howHot'] }}</span>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <ul class="list-group">
                        @foreach($newcomments as $newcomment)
                            <li class="list-group-item">
                                <div class="media">
                                    <div class="media-left">
                                        <a href="{{ action('ProfileController@index',['id'=>$newcomment->user->id]) }}">
                                            <img class="media-object img-thumbnail" src="{{ url($newcomment->user->avatar) }}" width="48">
                                        </a>
                                    </div>
                                    <div class="media-body">
                                        ...
                                        <p class="media-meta"><a href="{{ action('ProfileController@index',['id'=>$newcomment->user->id]) }}">{{ $newcomment->user->name }}</a>
                ...
                <div class="panel panel-default">
                    ...
                    <ul class="list-group">
                        <li class="list-group-item text-center height-115"><img src="{{ url('img/segmentfault.png') }}" alt="segmentfault""></li>
                        <li class="list-group-item text-center height-115"><img src="{{ url('img/cnodejs.png') }}" alt="cnodejs"></li>
                        <li class="list-group-item text-center height-115"><img src="{{ url('img/laravel.png') }}" alt="laravel" width="250px;"></li>
                    </ul>
                </div>
